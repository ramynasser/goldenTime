//
//  DetailsOfFLatController.swift
//  GoldenTime
//
//  Created by ramy nasser on 6/28/17.
...
class DetailsOfFLatController: UITableViewController {
    var proper:Property?
...
//                        
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "FlatsDetailsCell", for: indexPath) as! FlatsDetailsCell
        if indexPath.row % 2 == 0 {
            let detail = self.proper?.details[indexPath.row]

        cell.contentView.backgroundColor = UIColor(rgb: 0xAE8A32)
        cell.attributeLabel.text = detail?.key
            if let value = self.proper?.details[indexPath.row].value{
        ...
        cell.ValueLabel.text = "\(value)"
            ...
            let detail = self.proper?.details[indexPath.row]
            cell.contentView.backgroundColor = UIColor(rgb: 0xCAA03A)
            cell.attributeLabel.text = detail?.key
            if let value = self.proper?.details[indexPath.row].value{
